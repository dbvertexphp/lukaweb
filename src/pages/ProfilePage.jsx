...
const ProfilePage = () => {
    ...
    // 1. Fetch Profile Data with Parsing Logic
    const fetchProfile = async () => {
        try {
            setLoading(true);
            const res = await axios.get(`${API_URL}/api/user/get-profile`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            ...
            if (res.data.status && res.data.user) {
                ...
                // 🔥 SABSE IMPORTANT: Address agar string hai toh use parse karein
                let parsedAddr = u.address;
                if (typeof u.address === 'string' && u.address.trim() !== "") {
                    try {
                        parsedAddr = JSON.parse(u.address);
                    } catch (e) {
                ...
                setFormData({
                    ...
                    address: {
                        ...
                        building_name: parsedAddr?.building_name || '',
    ...
    const handleProfileUpdate = async (e) => {
        ...
        try {
            ...
            data.append('email', formData.email);
            data.append('mobile', formData.mobile);
            data.append('address', JSON.stringify(formData.address)); // Stringify for DB
            
            if (selectedFile) {
                data.append('profile_pic', selectedFile);
            }

            const res = await axios.post(`${API_URL}/api/user/updateCostomerProfileData`, data, {
                headers: { 
                    Authorization: `Bearer ${token}`,
                    "Content-Type": "multipart/form-data" 
                }
            ...
            if (res.data.status) {
                ...
                setEditMode(false);
                fetchProfile(); // Data refresh karein
            }
        } catch (error) {
            toast.error(error.response?.data?.message || "Update failed!", { id: loadId });
        }
    };
    ...
    const getProfileImg = () => {
        ...
        if (!user?.profile_pic) return `https://ui-avatars.com/api/?name=${user?.full_name || 'User'}&background=random`;
        ...
        <div className="min-h-screen bg-gray-50 pb-20 font-sans">
            ...
            <div className="max-w-6xl mx-auto px-4">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 -mt-24 relative z-10">
                    ...
                    {/* LEFT SIDEBAR */}
                    <div className="lg:col-span-4 space-y-6">
                        <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 flex flex-col items-center relative transition-all">
                            ...
                            <div className="text-center">
                                <h2 className="text-xl font-black text-[#1e3a5f] uppercase tracking-tight">{user?.full_name || 'User'}</h2>
                                <div className="mt-3">
                                    <span className="bg-blue-50 text-blue-600 px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border border-blue-100 italic">
                                        {user?.role || 'Customer'}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="w-full mt-10 pt-8 border-t border-gray-50 space-y-5">
                                <div className="flex items-center gap-4 text-gray-500"><Mail size={18} className="text-gray-300" /><p className="text-sm font-semibold truncate">{user?.email}</p></div>
                                <div className="flex items-center gap-4 text-gray-500"><Phone size={18} className="text-gray-300" /><p className="text-sm font-semibold">{user?.mobile || 'Not provided'}</p></div>
                            </div>
                        </div>
                        ...
                        <div onClick={() => navigate('/orders')} className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 flex justify-between items-center hover:bg-gray-50 cursor-pointer transition-all group">
                            ...
                            <span className="text-gray-300 group-hover:text-blue-600 group-hover:translate-x-1 transition-all">→</span>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-8">
                        <form onSubmit={handleProfileUpdate} className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-sm border border-gray-100 min-h-[500px]">
                            <div className="flex items-center justify-between mb-10 border-b border-gray-50 pb-6">
                                <h3 className="text-2xl font-black text-[#1e3a5f] uppercase tracking-tight italic">Account <span className="text-blue-600">Settings</span></h3>
                                {!editMode && <span className="text-[10px] font-bold text-gray-300 uppercase tracking-widest italic opacity-50">View Only Mode</span>}
                            </div>
                            
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Full Name</label>
                                    <input disabled={!editMode} type="text" value={formData.full_name} onChange={(e) => setFormData({...formData, full_name: e.target.value})} className="w-full bg-gray-50 border-2 border-transparent rounded-2xl px-5 py-4 text-sm font-bold text-[#1e3a5f] outline-none focus:bg-white focus:border-blue-100 transition-all disabled:opacity-50" />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Mobile Number</label>
                                    <input disabled={!editMode} type="text" value={formData.mobile} onChange={(e) => setFormData({...formData, mobile: e.target.value})} className="w-full bg-gray-50 border-2 border-transparent rounded-2xl px-5 py-4 text-sm font-bold text-[#1e3a5f] outline-none focus:bg-white focus:border-blue-100 transition-all disabled:opacity-50" />
                                </div>
                                <div className="md:col-span-2 space-y-2">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">City / Region</label>
                                    <input disabled={!editMode} type="text" value={formData.address.city} onChange={(e) => setFormData({...formData, address: {...formData.address, city: e.target.value}})} className="w-full bg-gray-50 border-2 border-transparent rounded-2xl px-5 py-4 text-sm font-bold text-[#1e3a5f] outline-none focus:bg-white focus:border-blue-100 transition-all disabled:opacity-50" />
                                </div>
                                <div className="md:col-span-2 space-y-2">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Flat / Building / House No.</label>
                                    <input disabled={!editMode} type="text" value={formData.address.building_name} onChange={(e) => setFormData({...formData, address: {...formData.address, building_name: e.target.value}})} className="w-full bg-gray-50 border-2 border-transparent rounded-2xl px-5 py-4 text-sm font-bold text-[#1e3a5f] outline-none focus:bg-white focus:border-blue-100 transition-all disabled:opacity-50" />
                                </div>
                            </div>

                            {editMode && (
                                <div className="mt-12 flex justify-end">
                                    <button type="submit" className="bg-[#4b70e2] text-white px-10 py-4 rounded-full font-black text-xs uppercase tracking-[0.2em] flex items-center gap-3 hover:bg-blue-700 transition-all shadow-xl shadow-blue-100 active:scale-95">
                                        <Save size={18} /> Save Changes
                                    </button>
                                </div>
                            )}
