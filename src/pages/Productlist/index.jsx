...
//             navigate(`/product/${product._id}`); // Checkout ki jagah Product Details par bhejein
//           }}  className="bg-[#1e3a5f] text-white w-full py-4 rounded-full font-black hover:bg-[#4b70e2] transition-all uppercase text-[10px] tracking-[0.2em] shadow-lg transform active:scale-95">
//           Buy Now
//         </button>
//       </div>
//     </div>
...
import Footer from "../../components/Footer";
import { toast, Toaster } from 'react-hot-toast';
import Product from "../products/index"; // 1. Detail component import kiya

const ProductListing = () => {
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [selectedProduct, setSelectedProduct] = useState(null); // 2. State banayi details load karne ke liye
  ...
  // 3. LOGIC: Agar product select hua hai toh wahi component dikhao (Home page ki tarah)
  if (selectedProduct) {
    window.scrollTo(0, 0); // Isse page hamesha top se khulega
  ...
  return (
    <div className="min-h-screen bg-white font-sans antialiased text-[#1e3a5f]">
      ...
      <section className="relative w-full h-[300px] md:h-[450px] bg-[#f8faff] flex items-center justify-center text-center px-4 overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-[#4b70e2]/5 rounded-full -mr-20 -mt-20 blur-3xl"></div>
        <div className="relative z-10">
          <h1 className="text-4xl md:text-7xl font-black text-[#1e3a5f] uppercase tracking-tighter italic mb-4">Our Collection</h1>
          <div className="w-24 h-2 bg-[#4b70e2] mx-auto rounded-full mb-6"></div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-6 py-24">
        {products.length === 0 ? (
          <div className="text-center py-20 opacity-20 font-black uppercase text-2xl italic tracking-tighter">No Products Found</div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 mb-32">
            {products.map((product) => (
              // 4. Pass setSelectedProduct to Card
              <ProductCard key={product._id} product={product} baseURL={baseURL} onSelect={() => setSelectedProduct(product)} />
            ))}
          </div>
        )}
        ...
        <section className="border-t border-gray-100 pt-24 pb-12 overflow-hidden relative">
          <div className="flex justify-between items-end mb-16">
            ...
            <div className="flex gap-4 pr-6">
              <button onClick={() => scroll('left')} className="w-12 h-12 rounded-full border-2 border-[#1e3a5f] flex items-center justify-center font-black">❮</button>
              <button onClick={() => scroll('right')} className="w-12 h-12 rounded-full border-2 border-[#1e3a5f] flex items-center justify-center font-black">❯</button>
            </div>
          </div>

          <div ref={scrollRef} className="flex gap-10 overflow-x-auto pb-12 no-scrollbar snap-x px-4 scroll-smooth">
            {products.slice(0, 6).map((product) => (
              <div key={`best-${product._id}`} className="min-w-[300px] md:min-w-[400px] flex snap-start">
                <ProductCard product={product} baseURL={baseURL} onSelect={() => setSelectedProduct(product)} />
              </div>
            ))}
          </div>
        </section>
...
const ProductCard = ({ product, baseURL, onSelect }) => {
  const imageUrl = product.product_images && product.product_images.length > 0 
    ? product.product_images[0].startsWith('http') 
        ? product.product_images[0] 
        : `${baseURL}/${product.product_images[0].replace(/^\/+/, "")}`
  ...
  const pPrice = product.price || 0;
  const pOldPrice = Math.round(pPrice * 1.3);

  return (
    <div 
      onClick={onSelect} // 5. Pure card click par detail khulega
      className="bg-white rounded-[3rem] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 cursor-pointer group border border-gray-100 flex flex-col relative"
    >
      <div className="absolute top-6 left-6 z-10 bg-[#d91e18] text-white text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-tighter shadow-lg italic">SALE -30%</div>
      
      <div className="h-80 overflow-hidden bg-[#f9f9f9] p-10 flex items-center justify-center">
        <img src={imageUrl} className="max-w-full max-h-full object-contain group-hover:scale-110 transition-transform duration-700 mix-blend-multiply" alt={product.product_name} />
      </div>
      
      <div className="p-10 text-center flex-grow flex flex-col justify-between">
        <div>
          <h3 className="text-[13px] font-black text-[#1e3a5f] mb-3 uppercase tracking-tighter line-clamp-2 h-10 leading-tight">{product.product_name}</h3>
          <div className="flex justify-center gap-1 mb-6">
             {[...Array(5)].map((_, i) => <span key={i} className="text-[#4b70e2] text-xs">★</span>)}
          </div>
          <div className="mb-8 flex flex-col items-center">
        ...
        <button 
          onClick={(e) => {
            ...
            onSelect(); // Buy Now par bhi detail khulega
          }} 
          className="bg-[#1e3a5f] text-white w-full py-4 rounded-full font-black hover:bg-[#4b70e2] transition-all uppercase text-[10px] tracking-[0.2em] shadow-lg transform active:scale-95"
        >
          Buy Now
